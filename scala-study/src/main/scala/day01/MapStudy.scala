...
object MapStudy {
  def main(args: Array[String]): Unit = {
    ...
    //返回一个对偶  Tuple
    for (a <- Map("Nick"-> 20,"Bob"-> 19)){
      println(a)
    }

    //返回一个对偶  Tuple (a._1) 返回 Nick Bob
    for (a <- Map("Nick"-> 20,"Bob"-> 19)){
      println(a._1)
    }

    for ((x,y) <- Map("Nick"-> 20,"Bob"-> 19)){
      println(x,y)
    }
    ...
    println("//返回符合指定条件的不可变 Map=================")
    println(combine_map.filterKeys(x=>{ //返回符合指定条件的不可变 Map;
      x.equals("test_100")
    }))

    println("查找集合中满足指定条件的第一个元素----------")
